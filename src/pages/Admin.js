...
function Admin() {
  ...
  useEffect(() => {
    ...
    chartInstanceRef.current = new Chart(ctx, {
        type: 'bar', data: { labels: ['Current Status'], datasets: [ { label: 'Checked In', data: [checkedIn], backgroundColor: 'rgba(16, 185, 129, 0.7)', borderColor: 'rgba(16, 185, 129, 1)', borderWidth: 1 }, { label: 'Checked Out', data: [checkedOut], backgroundColor: 'rgba(239, 68, 68, 0.7)', borderColor: 'rgba(239, 68, 68, 1)', borderWidth: 1 }, { label: 'Awaiting', data: [remaining], backgroundColor: 'rgba(209, 213, 219, 0.7)', borderColor: 'rgba(209, 213, 219, 1)', borderWidth: 1 } ] }, options: { indexAxis: 'y', responsive: true, maintainAspectRatio: false, scales: { x: { stacked: true, title: { display: true, text: 'Number of Students' } }, y: { stacked: true } }, plugins: { legend: { position: 'bottom' }, title: { display: false } } }
    });
  ...
  const handleDeleteStudent = async (id) => {
    ...
    try {
      ...
      // FIX: Use message state for feedback, not direct DOM manipulation or alert
  ...
  // ========================================================================
  // == MODAL COMPONENT (Consider moving to components/Modal.js) ==
  // ========================================================================
  ...
  // ========================================================================
  // == ADD/EDIT STUDENT FORM COMPONENT (Consider moving to components/AddEditStudentForm.js) ==
  // ========================================================================
  // This form handles both adding (selectedStudent is null) and editing (selectedStudent has data)
  const AddEditStudentForm = ({ studentData, onFormSubmit }) => {
    const [formData, setFormData] = useState({
      name: studentData?.name || "",
      grade: studentData?.grade || "",
      parentId: studentData?.parentId || "", // Parent ID might be complex object, handle appropriately
      status: studentData?.status || "awaiting" // Default new students to awaiting? Or checked-out?
      // Add other student fields if admin can edit them (emergency contacts etc?)
    });
...
useEffect(() => {
 ...
 // Removed studentData dependency, fetch parents once
    ...
    value={
      parents.find(p => p._id === formData.parentId)
        ? {
            ...
            label: `${parents.find(p => p._id === formData.parentId).name} (${parents.find(p => p._id === formData.parentId).email})`,
          }
        : null
    }
    onChange={(selected) =>
      setFormData((prev) => ({
        ...prev,
        parentId: selected ? selected.value : "",
      }))
    }
    options={parents.map((parent) => ({
      value: parent._id,
      label: `${parent.name} (${parent.email})`,
      isNew: new Date() - new Date(parent.createdAt) < 7 * 24 * 60 * 60 * 1000,
    }))}
    styles={{
      option: (base, state) => ({
        ...base,
        fontWeight: state.data.isNew ? "bold" : "normal",
        color: state.data.isNew ? "#065f46" : base.color,
      }),
    }}
  />
</div>
        ...
        <div className="flex justify-end space-x-3 pt-4">
          <button
            type="button"
            onClick={() => setShowModal(false)} // Use prop passed from Admin
    ...
    <div className="flex h-screen bg-gray-100">
      ...
      <aside className={`w-64 bg-white shadow-md flex flex-col fixed inset-y-0 left-0 z-30 overflow-y-auto transition-transform transform lg:translate-x-0 ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"} lg:static`}>
        ...
        <nav className="flex-1 px-2 py-4 space-y-1">
            {[ { id: 'dashboard-content', title: 'Dashboard', icon: faTachometerAlt }, { id: 'dismissal-content', title: 'Dismissal/Pickup', icon: faCar }, { id: 'students-content', title: 'Student Roster', icon: faUserGraduate }, { id: 'parents-content', title: 'Parents/Guardians', icon: faUserFriends }, { id: 'reports-content', title: 'Reports', icon: faChartLine }, { id: 'logs-content', title: 'Activity Logs', icon: faHistory } ].map(item => ( <a key={item.id} href={`#${item.id}`} className={`sidebar-link group flex items-center px-3 py-2.5 text-sm font-medium rounded-md ${activeSection === item.id ? 'bg-blue-50 text-blue-700 font-semibold' : 'text-gray-700 hover:bg-gray-100 hover:text-gray-900'}`} onClick={(e) => { e.preventDefault(); switchSection(item.id, item.title); }} > <FontAwesomeIcon icon={item.icon} className={`fa-fw mr-3 ${activeSection === item.id ? 'text-blue-600' : 'text-gray-400 group-hover:text-gray-500'}`} /> {item.title} </a> ))}
        </nav>
        <div className="p-4 border-t mt-auto space-y-2">
            <a href="#settings" className={`sidebar-link group flex items-center px-3 py-2.5 text-sm font-medium rounded-md ${activeSection === 'settings-content' ? 'bg-blue-50 text-blue-700 font-semibold' : 'text-gray-700 hover:bg-gray-100 hover:text-gray-900'}`} onClick={(e) => { e.preventDefault(); switchSection('settings-content', 'Settings'); }} > <FontAwesomeIcon icon={faCog} className={`fa-fw mr-3 ${activeSection === 'settings-content' ? 'text-blue-600' : 'text-gray-400 group-hover:text-gray-500'}`} /> Settings </a>
            <div className="pt-2 border-t"> <p className="text-sm font-medium text-gray-700">Admin Portal</p> <button onClick={handleLogout} className="text-xs text-red-600 hover:underline mt-1 w-full text-left"> <FontAwesomeIcon icon={faSignOutAlt} className="mr-1" /> Logout </button> </div>
        </div>
      </aside>
    ...
    <path strokeLinecap="round" strokeLinejoin="round" d="M4 6h16M4 12h16M4 18h16" />
            ...
            <div className="space-x-2">
              ...
              {activeSection === 'students-content' && (
                 <button onClick={handleAddStudent} className="bg-green-500 hover:bg-green-600 text-white font-medium py-1.5 px-3 rounded-md text-xs sm:text-sm transition duration-150 shadow-sm">
                    <FontAwesomeIcon icon={faPlus} className="mr-1" /> Add Student
                 </button>
              )}
              <button onClick={handleLogStudentAction} className="bg-blue-500 hover:bg-blue-600 text-white font-medium py-1.5 px-3 rounded-md text-xs sm:text-sm transition duration-150 shadow-sm">
            ...
            {loading ? (
              ...
              <>
                ...
                <div id="dashboard-content" className={`content-section ${activeSection !== 'dashboard-content' ? 'hidden' : ''}`}>
                   ...
                   <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-6">
                       {/* Stat Cards ... */}
                        ...
                        <div className="bg-white p-5 rounded-lg shadow border border-red-300 flex items-center"> <div className="bg-red-100 p-3 rounded-full"><FontAwesomeIcon icon={faExclamationTriangle} className="text-2xl text-red-600 animate-pulse"/></div> <div className="ml-4"><p className="text-sm font-medium text-red-600">Active Alerts</p><p className="text-3xl font-semibold text-red-700 mt-1">{stats.alerts}</p><p className="text-xs text-red-500 mt-1 hover:underline cursor-pointer">View Alerts</p></div></div>
                   </div>
                   {/* Chart & Activity Feed */}
                   <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                       ...
                       <div className="bg-white p-5 rounded-lg shadow border border-gray-200 flex flex-col">
                           ...
                           <div className="activity-feed flex-1 overflow-y-auto space-y-3 pr-2" style={{ maxHeight: "300px" }}>
                               {logs.length === 0 ? ( <p className="text-center text-gray-500 py-4">No recent activity</p> ) : ( logs.map((log) => { const styleInfo = getActivityStyles(log.type); const time = new Date(log.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }); const studentName = log.studentId?.name ?? "N/A"; const parentName = log.parentId?.name ?? ""; return ( <div key={log._id} className={`text-sm p-3 rounded ${styleInfo.style} border-l-4`}> <div className="flex items-start"> <FontAwesomeIcon icon={styleInfo.icon} className="mt-1 mr-2" /> <div className="flex-1"> <p className="font-medium">{log.type === "dropoff" ? "Drop-off" : "Pick-up"}:</p> <p>{studentName} by {parentName}</p> <p className="text-xs opacity-75 mt-1">{time}</p> </div> </div> </div> ); }) )}
                           </div>
                   ...
                   {/* Added Recent Students Table to Dashboard */}
                   <div className="bg-white p-5 rounded-lg shadow border border-gray-200 mb-6">
                       <div className="flex justify-between items-center mb-4"> <h3 className="text-lg font-semibold text-gray-800">Recently Updated Students</h3> <button onClick={() => switchSection('students-content', 'Student Roster')} className="text-sm text-blue-600 hover:text-blue-800 hover:underline" > View all students </button> </div>
                       <div className="overflow-x-auto">
                           <table className="min-w-full divide-y divide-gray-200">
                               <thead className="bg-gray-50"> <tr> <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Student</th> <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Grade</th> <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Status</th> <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Last Activity</th> </tr> </thead>
                               <tbody className="bg-white divide-y divide-gray-200"> {students.slice(0, 5).map((student) => ( <tr key={student._id} className="hover:bg-gray-50"> <td className="px-4 py-3 whitespace-nowrap"><div className="text-sm font-medium text-gray-900">{student.name}</div></td> <td className="px-4 py-3 whitespace-nowrap"><div className="text-sm text-gray-500">{student.grade}</div></td> <td className="px-4 py-3 whitespace-nowrap"><span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusInfo(student.status).style}`}> <FontAwesomeIcon icon={getStatusInfo(student.status).icon} className="mr-1" /> {getStatusInfo(student.status).text} </span></td> <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{formatDateTime(student.lastActivity)}</td> </tr> ))} </tbody>
                           </table>
                       </div>
                   </div>
                </div>
                ...
                <div id="dismissal-content" className={`content-section ${activeSection !== 'dismissal-content' ? 'hidden' : ''}`}>
                    {/* Removed h2/p - title is set by switchSection */}
                    <div className="bg-white p-6 rounded-lg shadow border border-gray-200 mb-6">
                        <h3 className="text-lg font-medium mb-4">Manual Check-in/Check-out</h3>
                        <p className="text-sm text-gray-600 mb-4"> Use this form to manually log student arrivals and departures. </p>
                        {/* Assuming ManualLogEntry component handles the form */}
                        <ManualLogEntry students={students} onLogSuccess={fetchData} />
                    </div>
                     {/* Add other dismissal management components here (Car Line, Bus Lists etc) */}
                </div>

                {/* Students Section */}
                <div id="students-content" className={`content-section ${activeSection !== 'students-content' ? 'hidden' : ''}`}>
                     <div className="flex justify-between items-center mb-4">
                         {/* Title is set by switchSection, maybe add subtitle */}
                         <p className="text-gray-600">Manage all students in the system.</p>
                         <button onClick={handleAddStudent} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-md text-sm font-medium flex items-center" > <FontAwesomeIcon icon={faPlus} className="mr-2" /> Add Student </button>
                     </div>
                     <div className="bg-white rounded-lg shadow border border-gray-200 mb-6">
                         <div className="p-4 border-b border-gray-200 flex flex-col sm:flex-row items-center justify-between gap-3">
                             ...
                             <div className="flex items-center space-x-3">
                                 <button className="px-3 py-2 border border-gray-300 rounded-md text-sm text-gray-700 bg-white hover:bg-gray-50 flex items-center"> <FontAwesomeIcon icon={faFilter} className="mr-2" /> Filter <FontAwesomeIcon icon={faChevronDown} className="ml-2 text-xs" /> </button>
                                 <button className="px-3 py-2 border border-gray-300 rounded-md text-sm text-gray-700 bg-white hover:bg-gray-50 flex items-center"> <FontAwesomeIcon icon={faFileDownload} className="mr-2" /> Export </button>
                             </div>
                         </div>
                         <div className="overflow-x-auto">
                             <table className="min-w-full divide-y divide-gray-200">
                                 <thead className="bg-gray-50"> <tr> <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Student Name</th> <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Grade</th> <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Status</th> <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Last Activity</th> <th className="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Actions</th> </tr> </thead>
                                 <tbody className="bg-white divide-y divide-gray-200">
                                     {filteredStudents.length > 0 ? filteredStudents.map((student) => ( <tr key={student._id} className="hover:bg-gray-50"> <td className="px-4 py-3 whitespace-nowrap"><div className="text-sm font-medium text-gray-900">{student.name}</div></td> <td className="px-4 py-3 whitespace-nowrap"><div className="text-sm text-gray-500">{student.grade}</div></td> <td className="px-4 py-3 whitespace-nowrap"><span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusInfo(student.status).style}`}> <FontAwesomeIcon icon={getStatusInfo(student.status).icon} className="mr-1" /> {getStatusInfo(student.status).text} </span></td> <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{formatDateTime(student.lastActivity)}</td> <td className="px-4 py-3 whitespace-nowrap text-sm font-medium"> <div className="flex justify-center space-x-4"> <button onClick={() => handleEditStudent(student)} className="text-indigo-600 hover:text-indigo-900" title="Edit"> <FontAwesomeIcon icon={faEdit} /> </button> <button onClick={() => handleDeleteStudent(student._id)} className="text-red-600 hover:text-red-900" title="Delete"> <FontAwesomeIcon icon={faTrash} /> </button> </div> </td> </tr> )) : (<tr><td colSpan="5" className="text-center py-10 text-gray-500">No students match your search.</td></tr>)}
                                 </tbody>
                             </table>
                         </div>
                         <div className="p-4 border-t border-gray-200 text-sm text-gray-600 flex justify-between items-center">
                             ...
                             {/* Add real pagination controls later */}
                             <div className="flex space-x-1"> <button className="px-3 py-1 border border-gray-300 rounded-md bg-white text-gray-500 text-xs">Previous</button> <button className="px-3 py-1 border border-gray-300 rounded-md bg-blue-50 text-blue-600 font-medium text-xs">1</button> <button className="px-3 py-1 border border-gray-300 rounded-md bg-white text-gray-500 text-xs">Next</button> </div>
                         </div>
                     </div>
                     {/* Removed StudentManager component instance */}
                </div>

                {/* Pending Approval Section */}
<div className="bg-white rounded-lg shadow border border-yellow-300 mb-6 mt-6">
    ...
    <table className="min-w-full divide-y divide-gray-200">
      ...
      <tbody className="bg-white divide-y divide-gray-100">
        {students.filter(s => s.approvalStatus === "pending").length === 0 ? (
          <tr><td colSpan="4" className="text-center py-4 text-gray-500">No students pending approval.</td></tr>
        ) : (
          students
      ...
      >
        ...
        {modalContent === "logEntry" ? (
          <ManualLogEntry students={students} onLogSuccess={() => { setShowModal(false); fetchData(); }} />
